...
const QuranReader = () => {
  ...
  useEffect(() => {
    ...
    // Click/Touch Outside listener: Hide immediately if tapping outside active ayah
    const handleOutsideClick = (e: MouseEvent | TouchEvent) => {
      const target = e.target as HTMLElement;
      if (!target.closest(`#ayah-${activeAyahMenu}`)) {
        setActiveAyahMenu(null);
        setShowHighlightMenu(null);
      }
    };
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-5 gap-1.5 sm:gap-2 pt-1">
                ...
                <button
                  onClick={() => toggleMarkVerse(activeAyahMenu)}
                  className={`flex flex-col items-center justify-center gap-1 p-2 rounded-2xl transition-all border ${
                    isMarked
