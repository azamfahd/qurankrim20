...
import { StartupPermissionOnboarding } from "./components/StartupPermissionOnboarding";
...
const DhikrSettingsModal = lazyWithRetry(
  () => import("./components/DhikrSettingsModal"),
  "DhikrSettingsModal"
);
const HistoryModal = lazyWithRetry(
  () => import("./components/HistoryModal"),
  "HistoryModal"
);
const TasbihModal = lazyWithRetry(
  () => import("./components/TasbihModal"),
  "TasbihModal"
);
const BookmarksModal = lazyWithRetry(
  () => import("./components/BookmarksModal"),
  "BookmarksModal"
);
const AdhkarModal = lazyWithRetry(
  () => import("./components/AdhkarModal"),
  "AdhkarModal"
);
const NamesOfAllahModal = lazyWithRetry(
  () => import("./components/NamesOfAllahModal"),
  "NamesOfAllahModal"
);
...
const AdhanSettingsModal = lazyWithRetry(
  () => import("./components/AdhanSettingsModal"),
  "AdhanSettingsModal"
);
const ManualLocationModal = lazyWithRetry(
  () => import("./components/ManualLocationModal"),
  "ManualLocationModal"
);
const HijriCalendarModal = lazyWithRetry(
  () => import("./components/HijriCalendarModal"),
  "HijriCalendarModal"
);
const QiblaModal = lazyWithRetry(
  () => import("./components/QiblaModal"),
  "QiblaModal"
);
const ZakatCalculatorModal = lazyWithRetry(
  () => import("./components/ZakatCalculatorModal"),
  "ZakatCalculatorModal"
);
const ProphetsModal = lazyWithRetry(
  () => import("./components/ProphetsModal"),
  "ProphetsModal"
);
const QuranPlatformModal = lazyWithRetry(
  () => import("./quran-platform/QuranPlatformModal"),
  "QuranPlatformModal"
);
const AgriculturalCalendarModal = lazyWithRetry(
  () => import("./components/AgriculturalCalendarModal")
);
const MiraclesModal = lazyWithRetry(() => import("./components/MiraclesModal"));
const InstallModal = lazyWithRetry(
  () => import("./components/InstallModal"),
  "InstallModal"
);
const BatteryOptimizationGuideModal = lazyWithRetry(
...
const App: React.FC = () => {
  ...
  const initialPreservedState = useRef(
    AppStatePreservation.getSavedState()
  ).current;
  ...
  const [isAdhanSettingsOpen, setIsAdhanSettingsOpen] = useState(
    () => initialPreservedState?.activeView === "adhan_settings"
  );
  const [isHistoryOpen, setIsHistoryOpen] = useState(
    () => initialPreservedState?.activeView === "history"
  );
  const [isTasbihOpen, setIsTasbihOpen] = useState(
    () => initialPreservedState?.activeView === "tasbih"
  );
  const [isDhikrReminderOpen, setIsDhikrReminderOpen] = useState(
    () => initialPreservedState?.activeView === "dhikr_settings"
  );
  const [isQuranPlatformOpen, setIsQuranPlatformOpen] = useState(
    () => initialPreservedState?.activeView === "quran"
  );
  const [quranInitialState, setQuranInitialState] = useState<{
    surah?: number;
    ayah?: number;
    view?: any;
  }>(() => {
  ...
  const [isBookmarksOpen, setIsBookmarksOpen] = useState(
    () => initialPreservedState?.activeView === "bookmarks"
  );
  const [isAdhkarOpen, setIsAdhkarOpen] = useState(
    () => initialPreservedState?.activeView === "adhkar"
  );
  const [isNamesOfAllahOpen, setIsNamesOfAllahOpen] = useState(
    () => initialPreservedState?.activeView === "names_of_allah"
  );
  const [isQiblaOpen, setIsQiblaOpen] = useState(
    () => initialPreservedState?.activeView === "qibla"
  );
  const [isLocationModalOpen, setIsLocationModalOpen] = useState(false);
  const [showLocationBanner, setShowLocationBanner] = useState(false);
  const [locationBannerData, setLocationBannerData] = useState<{
    location?: UserLocation;
    isHighAccuracy?: boolean;
  }>({});
  const [isZakatOpen, setIsZakatOpen] = useState(
    () => initialPreservedState?.activeView === "zakat"
  );
  const [isHijriOpen, setIsHijriOpen] = useState(
    () => initialPreservedState?.activeView === "hijri"
  );
  const [isAgriCalendarOpen, setIsAgriCalendarOpen] = useState(
    () => initialPreservedState?.activeView === "agri_calendar"
  );
  const [isMiraclesOpen, setIsMiraclesOpen] = useState(
    () => initialPreservedState?.activeView === "miracles"
  );
  const [isProphetsOpen, setIsProphetsOpen] = useState(
    () => initialPreservedState?.activeView === "prophets"
  );
  const [isAboutOpen, setIsAboutOpen] = useState(
    () => initialPreservedState?.activeView === "about"
  );
  ...
  const [apkUpdateInfo, setApkUpdateInfo] = useState<ApkVersionInfo | null>(null);
  ...
  useEffect(() => {
    ...
    const handleServiceWorkerMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === "STOP_ADHAN") {
        ...
        const hasInstalledApk = localStorage.getItem(
          "anis_apk_installed_version"
        );
    ...
    const checkAdhan = () => {
      ...
      if (!cachedSchedule || cachedScheduleDateKey !== dateKey) {
        cachedSchedule = calculateAccuratePrayerTimes(
          settings.location,
          now,
          adhanConfig.calculationMethod
        );
      ...
      schedule.prayersList.forEach((prayer: any) => {
        const isPrayerEnabled = adhanConfig[
          prayer.key as keyof typeof adhanConfig
        ] as boolean;
        ...
        if (
          rawDiffMinutes >= 0 &&
          rawDiffMinutes <= 2.5 &&
          !triggeredPrayersRef.current.has(triggerKey)
        ) {
          triggeredPrayersRef.current.add(triggerKey);
          try {
            sessionStorage.setItem(
              "anis_triggered_prayers_" + dateKey,
              JSON.stringify(Array.from(triggeredPrayersRef.current))
            );
          } catch {}
  ...
  useEffect(() => {
    ...
    const cleanupBack = AppStatePreservation.initHardwareBackButton(() => {
      ...
      const miracleSelected = localStorage.getItem(
        "anis_miracle_selected_category_id"
      );
      ...
      if (isDhikrReminderOpen) {
        setIsDhikrReminderOpen(false);
        return true;
      }
      if (isAdhanSettingsOpen) {
      ...
      if (isAgriCalendarOpen) {
        setIsAgriCalendarOpen(false);
        return true;
      }
      if (isHijriOpen) {
        setIsHijriOpen(false);
        return true;
      }
      if (isZakatOpen) {
      ...
      if (isQuranPlatformOpen) {
        setIsQuranPlatformOpen(false);
        return true;
      }
      if (isSidebarOpen) {
    ...
    const initLoad = async (uid: string, isLogged: boolean, user?: any) => {
      ...
      if (loadedSettings) {
        const effectiveLocation =
          loadedSettings.location || finalSettings.location || savedLocObj;
        const effectiveApiKey =
          loadedSettings.apiKey && loadedSettings.apiKey.trim().length > 0
        ...
        finalSettings = {
          ...finalSettings,
          username:
            user?.user_metadata?.full_name || settings.username || "مستخدم",
          email: user?.email || "",
      ...
      if (user) {
        ...
        const isGuest =
          ...
          (prevUserId.startsWith("guest_") ||
            localStorage.getItem("anis_is_guest") === "true");
        if (isGuest && prevUserId !== user.id) {
          ...
          try {
            ...
            await SupabaseService.migrateLocalGuestData(
              user.id,
              localSessions,
              localSettings
            );
  ...
  useEffect(() => {
    ...
    LocationService.detectLocationWithDetails(false)
      .then((details) => {
  ...
  useEffect(() => {
    ...
    if (!settings.isLoggedIn && !supabaseUser) {
      try {
        ...
        const currentSessionVisits = sessionStorage.getItem(
          "anis_session_visited"
        );
        ...
        const isDismissed =
          localStorage.getItem("anis_promo_banner_dismissed") === "true";
    ...
    if (settings.adhanSettings) {
      try {
        localStorage.setItem(
          "anis_adhan_settings",
          JSON.stringify(settings.adhanSettings)
        );
      } catch (e) {}
    }
  ...
  useEffect(() => {
    ...
    const handleAppInstalled = () => {
      if (userIdRef.current) {
        SyncService.updateUserInstallStatus(userIdRef.current, true).catch(
          console.error
        );
        showToast(
          "شكراً لتثبيت التطبيق! يمكنك الآن الوصول إليه من شاشتك الرئيسية.",
          "success"
        );
      }
    };
      ...
      if (lastMsg?.type === "ai") {
        ...
        setTimeout(() => {
          ...
          if (lastAiEl) {
            ...
            const y =
              lastAiEl.getBoundingClientRect().top +
              window.pageYOffset +
              yOffset;
  ...
  const saveCurrentSessionToHistory = (msgs: ChatMessage[]) => {
    ...
    if (currentSessionId) {
      ...
      if (existingSession) {
        ...
        setSessions((prev) =>
          prev.map((s) => (s.id === currentSessionId ? sessionToSave : s))
        );
      } else {
  ...
  const handleToggleBookmark = (verse: Verse) => {
    if (!verse.surahNumber || !verse.ayahNumber) {
      showToast(
        "عذراً، لا يمكن حفظ هذه الآية لعدم توفر بيانات السورة والآية.",
        "error"
      );
      return;
    }
    ...
    setSettings((prev) => {
      const isBookmarked = (prev.bookmarks || []).some(
        (b) =>
          b.verse.surahNumber === verse.surahNumber &&
          b.verse.ayahNumber === verse.ayahNumber
      );
  ...
  const handleEmotionSubmit = async (text: string) => {
    if (!isOnline) {
      setError(
        "لا يوجد اتصال بالإنترنت. يرجى تفعيل أو الاتصال بالإنترنت لاستخدام البحث والاستفسار."
      );
    ...
    const displayName =
      settings.username ||
      (settings.email ? settings.email.split("@")[0] : undefined);
    ...
    try {
      ...
      const data = await chatSessionRef.current.sendMessage(
        text,
        displayName,
        messages,
        onProgressUpdate
      ...
      let errorMessage =
        "عذراً، تعذر الاتصال بمحرك الذكاء الاصطناعي السحابي. يرجى التحقق من اتصالك بالإنترنت والمحاولة مجدداً للحصول على الإجابة الاحترافية الدقيقة.";
      if (err?.message) {
        ...
        if (msg.includes("quota") || msg.includes("429")) {
          ...
          errorMessage =
            "يبدو أن هناك مشكلة في استقرار الاتصال بالإنترنت. يرجى التحقق من اتصالك والمحاولة مرة أخرى.";
        } else if (
          msg.includes("استجابة فارغة") ||
          msg.includes("لم يتم العثور على استجابة")
        ) {
          errorMessage =
            "لم نتمكن من صياغة إجابة مناسبة في الوقت الحالي. يرجى إعادة صياغة سؤالك والمحاولة مرة أخرى.";
        } else if (msg.includes("timeout") || msg.includes("فشل الاتصال")) {
          errorMessage =
            "استغرق الخادم وقتاً طويلاً للاستجابة. يرجى المحاولة مرة أخرى لاحقاً.";
        } else if (msg.includes("json") || err.name === "SyntaxError") {
          errorMessage =
            "حدث خطأ في تنسيق البيانات الواردة من الذكاء الاصطناعي. يرجى المحاولة مرة أخرى.";
        } else {
          errorMessage = err.message.startsWith("تعذر")
            ? err.message
            : `عذراً، تعذر الاتصال بالذكاء الاصطناعي: ${err.message}`;
        }
      }
  ...
  const handleSilentRefresh = async () => {
    ...
    try {
      if (userIdRef.current && isOnline) {
        const loadedSettings = await SyncService.loadSettings(
          userIdRef.current,
          settings
        );
        if (loadedSettings) {
        ...
        const loadedSessions = await SyncService.loadSessions(
          userIdRef.current,
          settings
        );
        if (loadedSessions) {
  ...
  return (
    <div className="app-wrapper royal-gradient w-full max-w-[480px] mx-auto shadow-2xl relative selection:bg-[var(--color-gold)] selection:text-white">
      <AnimatePresence>
        {!isOnline && !isOfflineBannerDismissed && (
          ...
          >
            <div className="flex items-center justify-center gap-2.5 py-2 px-8">
              ...
              <span>
                أنت تعمل حالياً دون اتصال بالإنترنت • القرآن الكريم والتفاسير
                وكافة الأذكار متاحة بالكامل
              </span>
              <button
        ...
        {showPromoBanner && !settings.isLoggedIn && (
          ...
          >
            <div
              className="bg-white/95 backdrop-blur-xl border border-[var(--color-gold)]/40 p-5 rounded-[2rem] shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-3xl w-full pointer-events-auto text-right"
              dir="rtl"
            >
              <div className="flex gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed font-semibold">
                    أهلاً بك مجدداً في أنيس القلوب! هذه زيارتك الثالثة للبرنامج.
                    لتجنب فقدان جميع محادثاتك القيمة واستشاراتك ومحفوظاتك من
                    الآيات، ننصحك بربط حسابك بـ Google بضغطة زر واحدة لمزامنتها
                    سحابياً والوصول إليها بأمان من أي جهاز.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0 justify-end md:justify-start">
                ...
                <button
                  onClick={() => {
                    ...
                    setShowPromoBanner(false);
                    showToast(
                      "تم إخفاء التنبيه. يمكنك دائماً تسجيل الدخول من قائمة الإعدادات.",
                      "info"
                    );
                  }}
      ...
      >
        {isChatStarted ? (
          ...
          >
            <div className="flex items-center gap-2.5 sm:gap-4">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <h1 className="text-xl font-black royal-text-gradient leading-tight tracking-tight">
                    أنيس القلوب
                  </h1>
                </div>
            ...
            <div className="flex items-center gap-2">
              <AnimatePresence>
                {isSyncing && (
                  ...
                  >
                    <RefreshCw
                      size={12}
                      className="text-[var(--color-gold)] animate-spin"
                    />
              ...
              >
                <span className="text-sm font-bold text-white group-hover:text-[var(--color-gold-light)] transition-colors hidden sm:block pr-2">
                  {settings.username || "ضيف"}
                </span>
        ...
        <main
          className={
            isChatStarted
              ? "w-full max-w-7xl mx-auto px-2 sm:px-4 flex flex-col"
            ...
            {state === AppState.LOADING && (
              <div className="flex justify-center py-6">
                <div className="bg-white/90 backdrop-blur-md px-6 py-3.5 rounded-2xl shadow-sm flex items-center gap-4 border border-[var(--color-primary)]/10">
                  ...
                  <span className="text-sm font-bold text-[var(--color-primary-dark)] animate-pulse">
                    {loadingText}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-2 sm:mt-3 w-full animate-slide-up">
                ...
                <div className="order-2 lg:order-1 lg:col-span-7 xl:col-span-7 flex flex-col">
                  <div className="mb-3 sm:mb-4 text-center relative">
                    ...
                    <div className="absolute -top-14 left-1/2 -translate-x-1/2 w-36 h-36 opacity-30 pointer-events-none z-0 animate-float-3d perspective-1000">
                      <div
                        className="w-full h-full rounded-full bg-gradient-to-tr from-[var(--color-primary)] to-[var(--color-primary-light)] blur-2xl"
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      ...
                      <div className="flex justify-center mb-3.5 w-full max-w-sm lg:max-w-md px-1">
                        <button
                          onClick={() =>
                            openQuran(undefined, undefined, "index")
                          }
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, #022c22 0%, #065f46 45%, #023829 80%, #996515 100%)",
                            boxShadow:
                              "0 10px 24px -6px rgba(0, 0, 0, 0.55), 0 0 25px rgba(212, 175, 55, 0.25), inset 0 1px 2px rgba(255, 255, 255, 0.35), inset 0 -3px 0 rgba(0, 0, 0, 0.45)",
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3.5 relative z-10">
                            <div
                              ...
                              style={{
                                background:
                                  "linear-gradient(135deg, #f1e5ac 0%, #d4af37 50%, #996515 100%)",
                                boxShadow:
                                  "0 4px 12px rgba(0,0,0,0.4), inset 0 1.5px 2px rgba(255,255,255,0.8), inset 0 -2px 0 rgba(0,0,0,0.3)",
                                border: "1px solid rgba(255, 245, 200, 0.8)",
                              }}
                            >
                              <BookOpen
                                size={20}
                                className="text-[#022c22] drop-shadow-sm"
                              />
                            </div>
                            ...
                            <div className="text-right">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-950/90 border border-amber-400/50 text-amber-200">
                                  قراءة
                                </span>
                                <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-950/90 border border-emerald-400/50 text-emerald-200">
                                  تلاوة
                                </span>
                                <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-950/90 border border-teal-400/50 text-teal-200">
                                  تفسير
                                </span>
                              </div>
                            </div>
              ...
              <div className="mt-5 mb-5 sm:mt-7 sm:mb-6">
                <div className="flex items-center gap-3 mb-3.5 px-1">
                  ...
                  <h3 className="text-xs sm:text-sm font-bold text-[var(--color-gold)] uppercase drop-shadow-xs">
                    الوصول السريع
                  </h3>
                ...
                <div className="quick-actions-grid">
                  <div
                    ...
                    style={{
                      gridColumn: "1 / -1",
                      background:
                        "linear-gradient(135deg, #022c22 0%, #065f46 50%, #996515 100%)",
                      color: "white",
                      boxShadow:
                        "0 8px 20px -4px rgba(0,0,0,0.45), inset 0 1px 1.5px rgba(255,255,255,0.35)",
                    ...
                    <div className="flex items-center justify-between w-full relative z-10">
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, #f1e5ac, #d4af37)",
                            color: "#022c22",
                        ...
                        <div className="flex flex-col text-right">
                          <span className="text-sm sm:text-base md:text-lg font-black text-amber-100 tracking-wide leading-tight drop-shadow-xs">
                            المصحف الشريف الذكي
                          </span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[11px] sm:text-xs px-2 py-0.2 rounded-full bg-emerald-950/85 text-amber-200 border border-amber-400/50 font-bold">
                              قراءة
                            </span>
                            <span className="text-[11px] sm:text-xs px-2 py-0.2 rounded-full bg-emerald-950/85 text-emerald-200 border border-emerald-400/50 font-bold">
                              تلاوة
                            </span>
                            <span className="text-[11px] sm:text-xs px-2 py-0.2 rounded-full bg-emerald-950/85 text-teal-200 border border-teal-400/50 font-bold">
                              تفسير
                            </span>
                          </div>
                        </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-40">
            ...
            <div
              style={{
                ...
                paddingLeft:
                  "max(0.75rem, calc(0.75rem + var(--safe-area-left)))",
                paddingRight:
                  "max(0.75rem, calc(0.75rem + var(--safe-area-right)))",
