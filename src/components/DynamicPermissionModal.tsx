...
export const DynamicPermissionModal: React.FC = () => {
  ...
  const getIcon = (type: PermissionType) => {
    switch (type) {
      case 'notifications': return <BellRing size={32} className="text-amber-500" />;
      case 'location': return <MapPin size={32} className="text-emerald-500" />;
      case 'audio': return <Activity size={32} className="text-blue-500" />;
      case 'background': return <ShieldAlert size={32} className="text-purple-500" />;
      default: return <ShieldAlert size={32} className="text-slate-500" />;
    }
  };
  ...
  return (
    <AnimatePresence>
      {request && (
        <motion.div
          key="dynamic-permission-backdrop"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <motion.div
            key="dynamic-permission-container"
