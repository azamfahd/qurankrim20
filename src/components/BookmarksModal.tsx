...
import { X, Bookmark as BookmarkIcon, Trash2, Calendar, Play, Pause, Copy, Share2, WifiOff, BookOpen } from 'lucide-react';
  ...
  const toggleAudio = async (bookmark: Bookmark) => {
    ...
    if (playingId === bookmark.id) {
      ...
      const fetchAudioUrl = async (reciterId: string, surah?: number, ayah?: number, useFallback: boolean = false): Promise<string | null> => {
        if (!useFallback) {
          return getQuranAudioUrl(reciterId, bookmark.verse.number, surah, ayah);
        } else {
          try {
            if (surah && ayah) {
      ...
      const initialUrl = await fetchAudioUrl(activeReciter, bookmark.verse.surahNumber, bookmark.verse.ayahNumber);
      if (!initialUrl) {
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center border-t border-gray-50 pt-4">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button 
                              onClick={() => {
                                onOpenQuran?.(bookmark.verse.surahNumber, bookmark.verse.ayahNumber, 'reader');
                                onClose();
                              }}
                              className="w-9 h-9 rounded-full flex items-center justify-center text-gray-500 hover:text-[var(--color-primary)] hover:bg-[var(--color-primary-light)] transition-colors"
                              title="فتح في المصحف"
